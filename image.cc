...
void Image::load_tiff(string input_filename)
{
  ...
  if (tif) {
    ...
    // std::cout<<"made tiff size is "<<this->m_image_data[120000]<<std::endl;//<<m_image_data.size()
  }
}
...
void Image::save_tiff_grey_8bit(string output_filename) 
{
  ...
  // Close the file
  TIFFClose(output_image);
  // std::cout<<"saved a 8bit lenght folan file "<<this->m_image_data[120000]<<std::endl;
}
...
uint32_t * Image::getArray(){
  return &(this->m_image_data[0]);
}

void Image::buildFromArray(uint32_t * ptr ,int w, int h){
  this->m_image_data.clear();
  this->m_width = w;
  this->m_height = h;
  int numpixel = w*h;
  for ( int i = 0 ; i < numpixel ; i++){
    this->m_image_data.push_back(ptr[i]);
  }
}
void Image::mergeVerticals(uint32_t * other,int width,int height,int edge){
  // std::cout<<"merging "<<this->m_height<<"--"<<this->m_width<<"-- other party "<<width<<" "<<height<<"  "<<this->m_image_data.size()<<std::endl;
  // uint32_t * ptr = other.getArray();
  int tempHeight = this->m_height;
  this->m_height = this->m_height + height - (2*edge);
  int numpixel = height* width;
  for ( int i = 0 ; i < edge;i++){
    for( int j = 0 ; j < width ; j++){
      P(m_image_data,tempHeight-edge+i,j)=other[(i+edge)*width+ j];
    }
  }
  /*for ( int i = 0 ; i < width*edge ; i++){
    this->m_image_data[(tempHeight-edge)*width+i] = other[i+(edge*width)];
  }*/

  for ( int i = width*edge*2 ; i < numpixel; i++){
    this->m_image_data.push_back(other[i]);
  }
  // std::cout<<"merging "<<this->m_height<<"--"<<this->m_width<<"-- other party "<<width<<" "<<height<<"-----"<<this->m_image_data.size()<<std::endl;
}