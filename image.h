...
public:
  ...
  //void breakImage(int windowSize, int count, int index);
  //void mergeImage(int windowsSize, vector<Image> inputs);
  void mergeVerticals(uint32_t* other,int width,int height, int edge);
  void buildFromArray(uint32_t *ptr, int w, int h);
  uint32_t * getArray();
